@canany([
    ...
    <li class="list-none group" x-data="{ open: {{ request()->routeIs('admin.system*') ? 'true' : 'false' }} }">
        <button @click="open = !open"
                class="flex items-center gap-4 w-full px-4 py-2.5 rounded-xl transition-all duration-300
            {{ request()->routeIs('admin.system*') ? 'text-slate-900' : 'text-slate-500 hover:bg-slate-50' }}">

            <i class="ti ti-database-cog text-[20px] transition-colors
        {{ request()->routeIs('admin.system*') ? 'text-[#c02228]' : 'text-slate-300 group-hover:text-slate-500' }}"></i>

            <span class="flex-1 text-left font-bold text-[14px] tracking-tight">{{ __('System') }}</span>
        ...
        <div x-show="open" x-collapse x-cloak class="mt-1 ml-3 pl-4 border-l border-slate-100 space-y-0.5">
            ...
            @can('admin.system.logs')
                ...
                <a href="{{ route('admin.system.logs') }}"
                   class="{{ $subStyle }} {{ request()->routeIs('admin.system.logs') ? $activeSub : $inactiveSub }}">
                    {{ __('Audit Logs') }}
                </a>
            @endcan
            ...
            @can('admin.system.jobs')
                <a href="{{ route('admin.system.jobs') }}"
                   class="{{ $subStyle }} {{ request()->routeIs('admin.system.jobs') ? $activeSub : $inactiveSub }}">
                    {{ __('Jobs') }}
                </a>
            @endcan
        </div>
