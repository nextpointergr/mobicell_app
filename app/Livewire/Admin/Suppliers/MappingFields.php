...
class MappingFields extends AComponent
{

    public $options = []; // ['field_name' => ['is_hashable' => true, 'is_unique' => false]]
    public $supplier;
    public $xmlFields = [];
    public $dbFields = [];
    ...
    public function mount($supplier)
    {
        ...
        foreach ($existing as $map) {
            ...
            // ΚΡΙΣΙΜΟ: Φορτώνουμε και τα options για να εμφανίζονται τικαρισμένα στο UI
            $this->options[$map->source_field] = [
                'is_hashable' => (bool)$map->is_hashable,
                'is_unique' => (bool)$map->is_unique_check,
            ];
        }
    }
    ...
    public function saveMapping()
    {
        foreach ($this->mappings as $source => $target) {
            ...
            SupplierFieldMapping::updateOrCreate(
                ...
                [
                    'target_field' => $target,
                    'is_hashable' => $this->options[$source]['is_hashable'] ?? true,
                    'is_unique_check' => $this->options[$source]['is_unique'] ?? false,
                    'active' => true
                ]
            );
        }
        ...
        $this->dispatch('notify', 'Mappings Saved!');
    }
