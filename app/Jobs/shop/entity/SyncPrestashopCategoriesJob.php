...
class SyncPrestashopCategoriesJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $response = Prestashop::categories()
            ->limit($this->limit)
            ->offset($this->offset)
            ->since($this->since ?? '')
            ->get();
        ...
        $this->performUpsert('prestashop', 'categories', $items);
    ...
    private function finish(SyncRun $run): void
    {
        ...
        ExternalSync::updateOrCreate(
            ['source' => 'prestashop', 'entity' => 'categories'],
            ['last_synced_at' => now()]
        );
